...
public class GUIVenta extends javax.swing.JFrame {
    ...
    private void jButtonVerFacturaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonVerFacturaActionPerformed
        ...
        try {
            ...
            int nextIdx = gv.getNextVentaIndex(); 
            String idFact = "V" + nextIdx;
            LocalDate fechaEmi = LocalDate.now();
            String idCli = jFieldID.getText().trim();
            Cliente cli = clientesById.get(idCli);
            
            Factura factura = new Factura(idFact, fechaEmi, cli, new ArrayList<>(ventasEnFactura));
          
            PdfReportVisitor pdfVisitor = new PdfReportVisitor("src/main/java/ArchivosPersistencia/factura_visitor.pdf");
            factura.accept(pdfVisitor);  // esto escribirá el PDF
            // (el visitor ya cierra el document en su propio método close())
            
            Desktop.getDesktop().open(new File("src/main/java/ArchivosPersistencia/factura_visitor.pdf"));
